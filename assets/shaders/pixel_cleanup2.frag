...
bool isGrassColor(vec3 color)
{
    const float tolerance = 0.05f;
    return distance(color, darkGrass) < tolerance ||
           distance(color, mediumGrass) < tolerance ||
           distance(color, lightGrass) < tolerance;
}
...
void main()
{
    ...
    int centerType = grassType(center.rgb);
    ...
    for (int i = 0; i < 8; i++)
    {
        if (!isGrassColor(neighbors[i].rgb))
        {
            continue;
        }
    ...
    int strongestNeighborCount = max(darkCount, max(mediumCount, lightCount));

    if (sameTypeNeighbors <= 2 && strongestNeighborCount >= 5)
    {
